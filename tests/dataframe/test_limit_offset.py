...
daft.set_execution_config(use_experimental_distributed_engine=False)


@pytest.fixture(scope="session")
def tmp_data(tmp_path_factory):
    # FIXME by zhenchao 2025-07-18 16:52:59
    ...
    # return str(tmp_path)
    return "/opt/workspace/daft/data/test"
...
def test(tmp_data):
    ...
    df6 = df.select("id", "name").limit(517).filter(col("id") > 127).limit(99)
    # df6.explain(show_all=True)
    print(f"{df6.count_rows()}")
    # assert_result_num_rows(df6, 517)
