...
def global_limit(
    ...
    child_plan = local_limit(child_plan=child_plan, limit=remaining_skip + remaining_rows, offset=0)
    started = False
    while True:
        ...
        while len(materializations) > 0 and materializations[0].done():
            ...
            print(
                f">> remaining[skip={remaining_skip}, "
                f"rows={remaining_rows}, "
                f"partitions={remaining_partitions}, "
                f"tasks={task_num_rows}/{len(materializations)}]"
            )
        ...
        if started:
            print(
                ...
                f"rows={remaining_rows}, "
                f"partitions={remaining_partitions}, "
                f"tasks={len(materializations)}]"
            )
            if len(materializations) > 0:
                for i in range(len(materializations)):
                    print(f">> task[{i}]={materializations[i]}")
