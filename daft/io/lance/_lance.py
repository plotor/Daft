...
def read_lance(
    ...
    Args:
        uri: URI to the LanceDB table (supports remote URLs to object stores such as `s3://` or `gs://`)
    ...
    if use_native_reader:
        ...
        handle = ScanOperatorHandle.lance_scan(
            uri, file_format_config=file_format_config, storage_config=storage_config
        )
