...
def _lancedb_table_factory_function(
    lance_ds: "lance.LanceDataset", fragment_id: int, required_columns: Optional[List[str]]
) -> Iterator["PyRecordBatch"]:
    fragment = lance_ds.get_fragment(fragment_id)
    assert fragment is not None, ValueError(
        f"Unable to find lance fragment {fragment_id}"
    )
    return (
